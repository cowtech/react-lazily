...
async function main() {
  ...
  const apiVersion = await got(versionUrl, { json: true })
  const version = apiVersion.body.version

  console.log(`Downloading normalize.css ${version} ...`)
  const normalize = (await got(downloadUrl.replace('{version}', version))).body.replace(/\n+/g, '')
