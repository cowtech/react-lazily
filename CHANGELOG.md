### 2021-06-02 / 3.0.0-alpha.6

- fix: Bugfixes.

### 2021-06-02 / 3.0.0-alpha.5

- fix: Bugfixes.

### 2021-06-01 / 3.0.0-alpha.4

- feat: Renamed styles.

### 2021-06-01 / 3.0.0-alpha.3

- feat: Update React generation.

### 2021-06-01 / 3.0.0-alpha.2

- fix: Fixed build.

### 2021-05-25 / 3.0.0-alpha.1

- feat: Use fela for styling.
- chore: Removed ignored files.

### 2021-03-03 / 2.6.0

- chore: Updated dependencies.

### 2021-01-05 / 2.5.1

- chore: Rebuild.
- fix: Bugfix.

### 2021-01-04 / 2.5.0

- feat: Export as ESM.

### 2021-01-03 / 2.4.2

- chore: Updated linter config.

### 2021-01-02 / 2.4.1

- fix: Format fix.

### 2021-01-02 / 2.4.0

- chore: Updated dependencies, code and tools.

### 2020-08-30 / 2.3.1

- fix: Fix sitemap exporting.

### 2020-08-30 / 2.3.0

- feat: Version bump.

### 2020-08-30 / 2.2.2

- feat: Render generic components inside a portal.

### 2020-08-23 / 2.2.1

- fix: Added missing export.

### 2020-08-22 / 2.2.0

- feat: Added CSSVarName.

### 2020-08-22 / 2.1.0

- feat: Added linkStyle and onServer.

### 2020-06-15 / 2.0.0

- feat: Removed CSX and simplified styles. Allow className everywhere.

### 2020-06-10 / 1.10.5

- fix: Disable ribbon on smaller screens.

### 2020-06-02 / 1.10.4

- fix: Added needed iOS function

### 2020-06-02 / 1.10.3

- feat: Added custom class support to spinner.

### 2020-06-02 / 1.10.2

- fix: Update style.

### 2020-05-25 / 1.10.1

- feat: Added Shortcuts element to exports.

### 2020-05-25 / 1.10.0

- feat: Added Shortcuts element.

### 2020-05-17 / 1.9.3

- fix: Always make sure serviceWorker are supported.

### 2020-05-17 / 1.9.2

- fix: Fix iOS sizes.

### 2020-05-17 / 1.9.1

- fix: Fix generation.

### 2020-05-17 / 1.9.0

- fix: Minor label fix.
- feat: Improve iOS splash generation.

### 2020-05-16 / 1.8.0

- feat: Namespace CSS var.

### 2020-05-16 / 1.7.0

- feat: Improve iOS support.

### 2020-05-11 / 1.6.3

- fix: Minor style fix.

### 2020-05-10 / 1.6.2

- chore: Minor style fix.

### 2020-05-10 / 1.6.1

- fix: Fixed Sitemap issue.

### 2020-05-10 / 1.6.0

- feat: Updated code style and dependencies.

### 2020-05-09 / 1.5.2

- fix: Remove normalize sourcemap information.

### 2020-05-03 / 1.5.1

- chore: Update Apple devices.

### 2019-09-11 / 1.5.0

- feat: Added dictionary support for Apple Splash tags.

### 2019-09-04 / 1.4.2

- fix: Fixed top anchor animation.

### 2019-09-04 / 1.4.1

- fix: Minor fix.
- fix: Fixed top anchor handling.

### 2019-09-04 / 1.4.0

- chore: Include missing files.
- feat: Added sitemap typings.

### 2019-09-01 / 1.3.1

- chore(deps): Fixed peer dependencies.

### 2019-09-01 / 1.3.0

- feat: Updated dependencies and images.

### 2019-08-30 / 1.2.0

- feat: Have multiple favicons.

### 2019-07-21 / 1.1.1

- chore: Use createMemoizedComponent everywhere.

### 2019-07-21 / 1.1.0

- feat: Added createMemoizedComponent.

### 2019-03-28 / 1.0.1

- chore: Add memoized components names.

### 2019-03-18 / 1.0.0

- feat: Switched to React hooks where appropriate.
- chore: Update normalize.css update script.

### 2019-01-17 / 0.12.6

- chore: Lint fix.

### 2019-01-14 / 0.12.5

- fix: Improve type definitions.

### 2018-12-27 / 0.12.4

- fix: Remove unneeded method.

### 2018-12-27 / 0.12.3

- chore: Use React.PureComponent when possible.
- chore: Lint fix.

### 2018-12-27 / 0.12.2

- fix: Improved binding.

### 2018-12-27 / 0.12.1

- fix: Make property optional.

### 2018-12-27 / 0.12.0

- feat: Use React.memo everywhere.

### 2018-12-25 / 0.11.0

- feat: Improve NewVersionChecker reloading.

### 2018-12-24 / 0.10.1

- fix: Support SSR.

### 2018-12-24 / 0.10.0

- feat: Added IndexedDB storage utils.

### 2018-12-23 / 0.9.1

- chore: Updated dependencies.

### 2018-12-22 / 0.9.0

- feat: Specify react as peerDependency.

### 2018-12-08 / 0.8.3

- fix: New version checker iPhone X support.

### 2018-12-08 / 0.8.2

- fix: Support non-rectangular screens.

### 2018-12-08 / 0.8.1

- fix: Added missing rel attribute.

### 2018-12-08 / 0.8.0

- feat: Added Apple splash screen util.

### 2018-09-07 / 0.7.0

- feat: Added emptyBoundHandler.

### 2018-07-17 / 0.6.0

- feat: Removed local-storage module in favor of using localforage.
- chore: Code style.

### 2018-06-23 / 0.5.1

- fix: Removed debug line.

### 2018-06-23 / 0.5.0

- chore: Add prettier to dev dependencies.
- feat: Updated dependency.

### 2018-03-25 / 0.4.0

- feat: Adjusted media queries.

### 2018-02-20 / 0.3.4

- chore: Updated dependencies.

### 2018-02-19 / 0.3.3

- chore: Simplified code.

### 2018-01-19 / 0.3.2

- chore: Rebuild JS version.

### 2018-01-19 / 0.3.1

- fix: Rename confusing methods.

### 2018-01-19 / 0.3.0

- chore: Only ship JS version with TS declarations.

### 2018-01-16 / 0.2.20

- fix: Bugfix.

### 2018-01-16 / 0.2.19

- feat: Add onClick handler to Icon.

### 2018-01-15 / 0.2.18

- feat: Added regexQuote method.

### 2018-01-11 / 0.2.17

- fix: Bugfix.

### 2018-01-09 / 0.2.16

- feat: Improved BrowseHappy detection.

### 2018-01-09 / 0.2.15

- fix: Added loadScript DOM util.

### 2018-01-04 / 0.2.14

- chore: Rebuild JS version.

### 2018-01-04 / 0.2.13

- fix: Fixed TopAnchor SSR handling.

### 2018-01-04 / 0.2.12

- fix: Added missing files.

### 2018-01-04 / 0.2.11

- chore: Updated dependencies.

### 2018-01-04 / 0.2.10

- feat: Added debugName management function.

### 2018-01-02 / 0.2.9

- fix: Improve code.

### 2018-01-02 / 0.2.8

- fix: Improve code.

### 2018-01-02 / 0.2.7

- fix: Bugfix.

### 2017-12-30 / 0.2.6

- fix: Bugfix.

### 2017-12-30 / 0.2.5

- fix: Minor improvement.

### 2017-12-30 / 0.2.4

- feat: Added SSR support to BrowseHappy.

### 2017-12-27 / 0.2.3

- fix: Bugfix.

### 2017-12-27 / 0.2.2

- fix: Bugfix.

### 2017-12-27 / 0.2.1

- fix: Fixed dependency specification.

### 2017-12-27 / 0.2.0

- feat: Use utility classes, csx is now a dependency.

### 2017-12-26 / 0.1.5

- fix: Correctly render TopAnchor.

### 2017-12-26 / 0.1.4

- fix: Minor style fixes.

### 2017-12-25 / 0.1.3

- feat: Added LocalStorage handling.

### 2017-12-25 / 0.1.2

- fix: Minor style fix.

### 2017-12-24 / 0.1.1

- feat: Move prop-types to devDependencies.

### 2017-12-24 / 0.1.0

- feat: Initial version.
