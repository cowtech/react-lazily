...
export function generateAppleSplashTags(url, whitelist = []) {
    ...
    return sizes.map(({ label, width, height, ratio }) => {
        return (React.createElement("link", { "data-splash-id": env === 'development' ? label : null, key: [width, ratio, height].join('-'), media: `(device-width: ${width}px) and (device-height: ${height}px) and (-webkit-device-pixel-ratio: ${ratio})`, href: url.replace('SUFFIX', `${width * ratio}x${height * ratio}`) }));
    });
}
