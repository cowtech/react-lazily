...
export function generateAppleSplashTags(url, whitelist = []) {
    ...
    return sizes.map(({ label, width, height, ratio }) => {
        ...
        return (React.createElement("link", { key: [width, ratio, height].join('-'), rel: "apple-touch-startup-image", media: `(device-width: ${width}px) and (device-height: ${height}px) and (-webkit-device-pixel-ratio: ${ratio})`, href: href, "data-splash-id": env === 'development' ? label : null }));
    });
}
