...
export function generateAppleSplashTags(url, includeLandscape = true, whitelist = []) {
    ...
    for (const { devices, width, height, ratio } of sizes) {
        ...
        const tags = [
            React.createElement("link", { key: portraitKey, rel: "apple-touch-startup-image", media: portraitQuery, href: portraitHref, "data-splash-id": id })
        ];
        if (includeLandscape) {
            const [landscapeKey, landscapeQuery, landscapeHref] = splashParams(height, width, ratio, url, 'landscape');
            tags.push(React.createElement("link", { key: landscapeKey, rel: "apple-touch-startup-image", media: landscapeQuery, href: landscapeHref, "data-splash-id": id }));
        }
    }
    return tags;
}
